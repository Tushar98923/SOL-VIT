...
class RouteFinder:
    ...
    def find_routes(self, source, destination):
        ...
        # === Check 2-hop Connection (source → stop1 → stop2 → destination) ===
        if source in self.graph:
            for stop1 in self.graph[source]:
                if stop1 in self.graph:
                    for stop2 in self.graph[stop1]:
                        if stop2 in self.graph and destination in self.graph[stop2]:
                            for route1 in self.graph[source][stop1]:
                                for route2 in self.graph[stop1][stop2]:
                                    for route3 in self.graph[stop2][destination]:
                                        possible_routes.append([
                                            {
                                                "mode": route1["mode"],
                                                "source": source,
        ...
        if len(possible_routes) > 5:
            possible_routes = random.sample(possible_routes, 5)

        return possible_routes

    def print_routes(self, possible_routes):
        """Print available routes for the user."""
        ...
        for idx, route_set in enumerate(possible_routes, start=1):
            ...
            for route in route_set:
                route_str += f"{route['mode'].capitalize()} from {route['source']} to {route['destination']} | "
                route_str += f"Cost: {route['cost']}, Duration: {route['duration']} mins\n"
                total_cost += route["cost"]
                total_duration += route["duration"]

            print(f"{idx}. {route_str.strip()} - Total Cost: {total_cost}, Total Duration: {total_duration} mins")

    def choose_route(self, possible_routes):
        ...
        try:
            choice = int(input("\nChoose a route by entering the corresponding number: "))
            if 1 <= choice <= len(possible_routes):
                selected_route = possible_routes[choice - 1]
                print("\n✅ You selected the following route:")
...
# Ask for source and destination inputs
source = input("Enter starting location: ").strip()
destination = input("Enter destination: ").strip()

# Get possible routes with direct and multi-mode options
possible_routes = route_finder.find_routes(source, destination)

# Show available routes and ask the user to choose one
route_finder.print_routes(possible_routes)
route_finder.choose_route(possible_routes)
