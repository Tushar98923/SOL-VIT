...
while True:
    try:
        choice = int(input("\nEnter the number of the route you want to select (1-5) or enter 0 to exit: "))
        
        if choice == 0:
            print("[INFO] Exiting the program. Goodbye!")
            break
        elif 1 <= choice <= len(possible_routes):
            selected_route = possible_routes[choice - 1]
            route_finder.print_full_route_details(selected_route)
        else:
            print("[ERROR] Invalid selection. Please choose a number between 1 and 5, or enter 0 to exit.")
    except ValueError:
